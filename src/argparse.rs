...
pub fn get_args() -> ArgMatches {
    let app = Command::new(crate_name!())
        ...
        // Only one of
        .arg(
            arg!(--"128" "128 bit, defaults to key length when not specified")
                .conflicts_with_all(["aes128", "aes192", "aes256"]),
        )
        .arg(
            arg!(--"192" "192 bit, defaults to key length when not specified")
                .conflicts_with_all(["aes128", "aes192", "aes256"]),
        )
        .arg(
            arg!(--"256" "256 bit, defaults to key length when not specified")
                .conflicts_with_all(["aes128", "aes192", "aes256"]),
        )
